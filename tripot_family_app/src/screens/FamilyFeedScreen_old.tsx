...
const FamilyFeedScreen: React.FC<FamilyFeedScreenProps> = ({
  ...
  useEffect(() => {
    ...
    const normalized = Object.entries(grouped).map(([date, photos]) => ({
      title: date,
      // data: chunkArray(photos, 3), // 3개씩 묶기
      // data: chunkArray(photos, 2),  // 2개씩 묶기
      data: photos,
    }));
    console.log('✅ [sections] normalized:', normalized); // 이 부분 추가!
    setSections(normalized);
  }, [feedData]);

  const renderRow = ({ item: photoRow }: any) => {
    console.log('📸 photoRow:', photoRow);
      ...
      <View style={styles.row}>
        {photoRow.map((photo: any) => {
          ...
          return (
            ...
            //   <Image
            //     source={{ uri: `${apiBaseUrl}${photo.file_url}` }}
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (!photo?.id) {
                  Alert.alert('오류', '사진 ID가 없습니다.');
                  return;
                }
                  navigation.openDetail(photo);
              }}
            >
              <Image
                // source={{ uri: `${apiBaseUrl}${photo.file_url}` }}
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <SectionList
          ...
          // keyExtractor={(item, index) => item.map((p: any) => p.id).join('-') || `${index}`}
          // keyExtractor={(item, index) =>
          //   Array.isArray(item) ? item.map((p) => p.id).join('-') : index.toString()
          // }
          keyExtractor={(item, index) => {
            const photoIds = Array.isArray(item) ? item.map((p) => p.id).join('-') : '';
          ...
          onRefresh={onRefresh}
          refreshing={loading}
          extraData={apiBaseUrl}
        />

        // <SectionList
        //   sections={sections}
        //   keyExtractor={(item, index) => `${item.id}-${index}`}
        //   renderItem={renderItem} // 변경된 렌더 함수
        //   renderSectionHeader={({ section: { title } }) => (
        //     <Text style={styles.sectionTitle}>{title}</Text>
        //   )}
        //   contentContainerStyle={{ paddingBottom: 80 }}
